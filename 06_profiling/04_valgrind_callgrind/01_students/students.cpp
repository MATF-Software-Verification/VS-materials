...
class student {
  public:
    student(string _name, string _surname) : name(_name), surname(_surname) {}
    void addGrade(string subject, int grade) {
        grades[subject].push_back(grade);
    }

    const map<string, vector<int>> &getGrades() const { return grades; }
    ...
    double getGPA(const string &subject) const {
        ...
        for (vector<int>::const_iterator i = getSubjectGrades(subject).begin();
    ...
    double getGPA() const {
        ...
        for (map<string, vector<int>>::const_iterator k = grades.begin();
             k != grades.end(); k++) {
...
void printClassNames(const vector<student> Class) {
    ...
    for (unsigned i = 0; i < Class.size(); i++)
        cout << Class[i].getName() << " " << Class[i].getSurname() << endl;
}
...
void printStudentGrades(vector<student> Class, const string name,
    ...
    for (map<string, vector<int>>::const_iterator kv = i->getGrades().begin();
         kv != i->getGrades().end(); kv++) {
        cout << kv->first << " : ";
...
void printClass(vector<student> Class) {
    ...
    for (vector<student>::iterator i = Class.begin(); i != Class.end(); i++)
