...
unsigned char valid_ubarray(ubarray *a) {
    return a != NULL && a->size >= 0 && a->bound > 0 && a->size <= a->bound &&
           a->data != NULL;
}
...
int uba_resize(ubarray *a, int new_bound) {
    if (new_bound > MAX_ARRAY_BOUND || new_bound <= 0)
        handle_error_en(ERR_INVALID_BOUND, "invalid new bound");
...
int ubarray_harness() {
    ...
    int B = nondet_int();
    ubarray *a = ubarray_new(B);
    if (a == NULL)
        handle_error_en(ERR_NULL_PTR, "Non alocated array");

    __CPROVER_assert(valid_ubarray(a), "valid after creating");

    int i;
    for (i = 0; i <= B / 2; i++)
        uba_add(a, nondet_char());

    __CPROVER_assert(valid_ubarray(a), "valid after adding");

    for (i = 0; i < uba_size(a); i++) {
        elem x = uba_get(a, i);
        uba_set(a, i, x + 2);
    }
    __CPROVER_assert(valid_ubarray(a), "valid after get-set");

    int newB = nondet_int();
    uba_resize(a, newB);

    __CPROVER_assert(valid_ubarray(a), "valid after resize");

    int size = uba_size(a);
    for (i = 0; i < size; i++)
        uba_rem(a);

    __CPROVER_assert(valid_ubarray(a), "valid after rem");
...
void queue_harness() {
    ...
    queue_t q = queue_new(B);
    if (q == NULL)
        handle_error_en(ERR_NULL_PTR, "Not allocated");
    ...
    __CPROVER_assert(!queue_empty(q), "Not empty queue");

    for (i = 0; i < B; i++)
        int x = deq(q);

    __CPROVER_assert(queue_empty(q), "Empty queue");
...
void queue_copy_harness() {
    ...
    queue_t new_q = queue_copy(q);
    __CPROVER_assert(!queue_empty(new_q), "Not empty queue");

    for (i = 0; i < B; i++) {
        int x = deq(new_q);
    }

    __CPROVER_assert(queue_empty(new_q), "Empty new queue");
