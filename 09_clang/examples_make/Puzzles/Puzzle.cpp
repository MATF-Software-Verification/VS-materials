...
void Puzzle::solve() {
    ...
    for (i = 0; i < 9; i++) // i is row
    {
        ...
        {
            if ((board[i][j]) !=
                0) // if there is already a value set possible to 2
            {
    ...
    while (!done()) // loop until solved
    {
        ...
        for (i = 0; i < 9; i++) // i is row
        {
            ...
            {
                if (possible[i][j][0] == 2)
                    continue; // if full skip value
                numpossible = 0;
                for (k = 0; k < 9; k++) // k+1 is value
                {
                    if (possible[i][j][k] == 1) {
                        numpossible++;
                        newval = k + 1; // if a number is possible store its
                                        // value in newval
                    }
                    if (numpossible > 1)
                        break;
                ...
                if (numpossible == 1) {
                    fill(newval, i,
                         j); // call fill to insert only possible value
                }
            }
        }
        ...
        for (i = 0; i < 9; i++) // i is row
        {
            for (k = 0; k < 9; k++) // k+1 is value
            {
                ...
                {
                    if (board[i][j] == k + 1)
                        break; // value in column already skip to next value
                    if (possible[i][j][k] == 1) {
                        numpossible++;
                        col = j;
                    }
                    if (numpossible > 1)
                        break; // call fill to insert value
                }
                if (numpossible == 1) {
                    // cout<<"column search: ";
                    fill(k + 1, i, col);
            ...
            for (j = 0; j < 9; j += 3) // j is column
            {
                for (k = 0; k < 9; k++) // k+1 is value
                {
                    ...
                    for (l = 0; l < 3; l++) // l is row
                    {
                        ...
                        {
                            // cout<<"checking location:"<<i+l<<","<<j+m<<" for
                            // a "<<k+1<<endl;
                            if (board[i + l][j + m] == k + 1)
                                break; // value in block already skip to next
                                       // value
                            if (possible[i + l][j + m][k] == 1) {
                                numpossible++;
                                row = i + l;
                                col = j + m;
                                // cout<<"Numpossible incremeneted for "<<k+1<<"
                                // at "<<row<<","<<col<<endl;
                            }
                            if (numpossible > 1)
                                break;
                    ...
                    if (numpossible == 1) // if only one possible position
                    {
                        ...
                        fill(k + 1, row,
                             col); // insert value in correct location in block
                    }
                }
            }
        }
        ...
        for (j = 0; j < 9; j++) // j is column
        {
            for (k = 0; k < 9; k++) // k+1 is value
            {
                if (possible[i][j][k] == 1) // if it is currently possible
                {
                    ...
                    {
                        if (l == j)
                            continue; // if on the value don't do anything
                    ...
                    for (l = ((i / 3) * 3); l < ((i / 3) * 3 + 3);
                         l++) // l is row
                    {
                        for (m = ((j / 3) * 3); m < ((j / 3) * 3 + 3);
                             m++) // m is coumn
                        {
                            if (l == i && m == j)
                                continue; // if on the actual value don't do
                                          // anything
                            if (board[l][m] == (k + 1)) {
                                possible[i][j][k] =
                                    0; // if a value is found in that 3x3 set
                                       // not possible (0)
                                break;
                            }
                        }
...
int Puzzle::done() {
    ...
    for (i = 0; i < board.size(); i++) {
        for (j = 0; j < board[i].size(); j++) {
            if (board[i][j] == 0)
                return 0; // if there is an empty value return not done(0)
        }
    }
    return 1; // if it cycles through whole puzzle finding no empty return
              // done(1)
}
